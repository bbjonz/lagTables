...
#' @param dname provide a name for the data set for table caption
...
lagmodels <- function(d, lagcol, laggroup, title="Lag Sequential Log Linear Models", lagnum=1) {
  ...
  if(missing(laggroup)) {
    ...
    # fit.out <- lag.counts %>%
    #   dplyr::group_by_at(vars(one_of(laggroup))) %>%
    #   dplyr::do(lagmodel=anova(glm(lag.form, data =., family = poisson)), test="Chisq")
    # 
    # 
    # fit.out %>%
    #  broom::tidy(lagmodel)
